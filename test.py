...
from scorer import CTCWithGPT2LM
import os
...
class TestDecoders(unittest.TestCase):
    def setUp(self):
        ...
        self.beam_search_result = ['acdc', "b'a", "a a", "bda"]
    ...
    def test_beam_search_decoder_3(self):
        ...
        lm_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'quant_gpt2.onnx')
        probs_seq = torch.FloatTensor([self.probs_seq2])
        decoder = CTCWithGPT2LM(self.vocab_list, beam_width=self.beam_size,
                                blank_id=self.vocab_list.index('_'),model_path=lm_path,vocab_path=vocab_path)
        beam_result, beam_scores, timesteps, out_seq_len = decoder.decode(probs_seq)
        output_str = self.convert_to_string(beam_result[0][0], self.vocab_list, out_seq_len[0][0])
