...
def download_extract(url, dl_path):
    ...
    tar.extractall('ctcdecode/third_party/')
    tar.close()
...
download_extract('https://github.com/parlance/ctcdecode/releases/download/v1.0/openfst-1.6.7.tar.gz',
                 'ctcdecode/third_party/openfst-1.6.7.tar.gz')
...
for file in ['ctcdecode/third_party/kenlm/setup.py', 'ctcdecode/third_party/ThreadPool/ThreadPool.h']:
    if not os.path.exists(file):
        warnings.warn('File `{}` does not appear to be present. Did you forget `git submodule update`?'.format(file))

# Does gcc compile with this header and library?
def compile_test(header, library):
    dummy_path = os.path.join(os.path.dirname(__file__), "ctcdecode/dummy")
...
lib_sources = glob.glob('ctcdecode/third_party/kenlm/util/*.cc') + glob.glob('ctcdecode/third_party/kenlm/lm/*.cc') + glob.glob(
    'ctcdecode/third_party/kenlm/util/double-conversion/*.cc') + glob.glob('ctcdecode/third_party/openfst-1.6.7/src/lib/*.cc')

lib_sources = [fn for fn in lib_sources if not (fn.endswith('main.cc') or fn.endswith('test.cc'))]

third_party_includes = [os.path.realpath(os.path.join("ctcdecode/third_party", lib)) for lib in third_party_libs]
ctc_sources = glob.glob('ctcdecode/ctcdecode/src/*.cpp') + glob.glob('neural_scorer.cpp')
